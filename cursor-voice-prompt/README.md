...
### 语音完全无转写、但希望仍能写入 Cursor

应用内提供 **「手动输入」** 文本框与 **「注入到焦点」**：在 Cursor 输入框聚焦后，可把整段 Prompt 粘贴进文本框再注入（**不经过** Web Speech，仅走剪贴板 + 模拟粘贴）。  
**「测试粘贴」** 用于确认注入链路是否正常：若测试成功而语音仍失败，即可判定为 **语音识别网络/TLS** 问题而非本机粘贴逻辑。
